...
class KnapsackEvaluator:
    ...
    def _get_backward_memory_from_topologically_sorted_graph(
        ...
        for node in sorted_nodes:
            ...
            if dependencies_computed:
                current_memory.append(
                    (
                        current_memory[-1][0]
                        - sum(
                            node_memories[recomputed_dependency]
                            for recomputed_dependency in dependencies_computed
                        ),
                        f"Dropping Predecessors of {node}: {dependencies_computed}",
                    )
                )
    ...
    def evaluate_knapsack_output(
        ...
        account_for_recomputation: bool = False,
    ) -> Dict[str, float]:
        """
        ...
        return {
            ...
            "peak_memory(GB)": peak_memory / 1024 / 1024 / 1024,
            "recomputation_runtime": recomputation_runtime,
            "non_ac_peak_memory": self._graph_info_provider.get_non_ac_peak_memory(),
            "non_ac_peak_memory(GB)": self._graph_info_provider.get_non_ac_peak_memory()
            / 1024
            / 1024
            / 1024,
